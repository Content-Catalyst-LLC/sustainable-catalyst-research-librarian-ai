...
SCRIPT_REVISION="v641-py312-retrieval-calibration-r1"
REPO_URL="git@github.com:Content-Catalyst-LLC/sustainable-catalyst-research-librarian-ai.git"
ZIP_PATH="${1:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-v6.4.1.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v641-clean}"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v641.XXXXXX")"
...
[[ -f "$SOURCE_DIR/tests/hybrid-retrieval-citation-contract-test.php" ]] || fail "The inherited v6.4.0 hybrid retrieval contract test is missing."
[[ -f "$SOURCE_DIR/tests/retrieval-calibration-regression-contract-test.php" ]] || fail "The v6.4.1 retrieval calibration contract test is missing."
[[ -f "$SOURCE_DIR/data/research_librarian_hybrid_retrieval_manifest_v6.4.0.json" ]] || fail "The inherited v6.4.0 hybrid retrieval manifest is missing."
[[ -f "$SOURCE_DIR/data/research_librarian_retrieval_calibration_manifest_v6.4.1.json" ]] || fail "The v6.4.1 calibration manifest is missing."
[[ -f "$SOURCE_DIR/data/research_librarian_retrieval_benchmarks_v6.4.1.json" ]] || fail "The v6.4.1 benchmark dataset is missing."
[[ -f "$SOURCE_DIR/docs/V641_RETRIEVAL_CALIBRATION_REGRESSION.md" ]] || fail "The v6.4.1 release documentation is missing."
[[ -f "$SOURCE_DIR/pytest.ini" ]] || fail "The package does not contain repository-root pytest configuration."
grep -q 'Version: 6.4.1' "$SOURCE_DIR/sustainable-catalyst-research-librarian-ai.php" || fail "The package version marker is not 6.4.1."
grep -q 'SCHEMA_VERSION = 6' "$SOURCE_DIR/backend/app/store.py" || fail "The package does not declare SQLite schema version 6."
grep -q 'sc-research-librarian-knowledge-index/6.0' "$SOURCE_DIR/backend/app/store.py" || fail "The package does not declare knowledge index schema 6.0."
grep -q 'SC_RL_STARTUP_WARMUP_SECONDS' "$SOURCE_DIR/render.yaml" || fail "The Render cold-start setting is missing."
grep -q 'SC_RL_SEMANTIC_ENABLED' "$SOURCE_DIR/render.yaml" || fail "The Render semantic retrieval setting is missing."
grep -q '@app.post("/v1/retrieve/explain"' "$SOURCE_DIR/backend/app/main.py" || fail "The retrieval explanation endpoint is missing."
grep -q 'def verify_citations' "$SOURCE_DIR/backend/app/provider.py" || fail "The citation verifier is missing."
grep -q 'def evidence_gate' "$SOURCE_DIR/backend/app/calibration.py" || fail "The minimum-evidence gate is missing."
grep -q '@app.post("/v1/retrieval/benchmark"' "$SOURCE_DIR/backend/app/main.py" || fail "The retrieval benchmark endpoint is missing."
grep -q 'retrieval_benchmark_runs' "$SOURCE_DIR/backend/app/store.py" || fail "The benchmark history table is missing."
...
run_clean "$VENV_PY" -m pytest -q backend/tests
...
git tag -a "$RELEASE_TAG" -m "Research Librarian v${RELEASE_VERSION} — Retrieval Calibration and Regression Patch"
