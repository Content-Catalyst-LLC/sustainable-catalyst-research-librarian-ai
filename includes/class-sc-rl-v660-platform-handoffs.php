...
final class SC_RL6_V660_Platform_Handoffs {
    ...
    private static function capability( $id, $label, $url, $enabled, $version, $accepts, $returns ) {
        $available = '1' === (string) $enabled && '' !== trim( (string) $url );
        return array(
            'id' => sanitize_key( $id ),
            'label' => sanitize_text_field( $label ),
            'available' => $available,
            'state' => $available ? 'configured' : 'disabled',
            'url' => esc_url_raw( $url ),
    ...
    private static function backend_request( $path, $method = 'GET', $payload = null ) {
        ...
        if ( '1' !== (string) $options['enabled'] || empty( $options['backend_url'] ) || empty( $options['backend_api_key'] ) ) {
            return new WP_Error( 'sc_rl_v660_backend_disabled', 'The Python backend is not configured.', array( 'status' => 503 ) );
        }
        $args = array(
        ...
        if ( $code < 200 || $code >= 300 || ! is_array( $body ) ) {
            ...
            return new WP_Error( 'sc_rl_v660_backend_failed', is_string( $detail ) ? sanitize_text_field( $detail ) : 'The typed-handoff backend request failed.', array( 'status' => $code ? $code : 502 ) );
        }
        return $body;
    }
        ...
        if ( ! is_wp_error( $backend ) && ! empty( $backend['capabilities'] ) && is_array( $backend['capabilities'] ) ) {
            foreach ( $backend['capabilities'] as $item ) {
                ...
                if ( $id && isset( $local[ $id ] ) ) {
                    ...
                    $local[ $id ]['state'] = $local[ $id ]['available'] ? 'available' : 'unavailable';
                }
            }
        }
    ...
    private static function local_handoff( $params ) {
        ...
        $payload = array(
            ...
            'boundaries' => array( 'This handoff is a reviewable draft.', 'The destination must validate its contract before accepting the payload.' ),
    ...
    public static function validate_payload( $payload ) {
        $errors = array();
        foreach ( array( 'schema', 'handoff_id', 'created_utc', 'source_system', 'source_version', 'question', 'route', 'destination', 'payload', 'provenance' ) as $field ) {
        ...
        return array( 'ok' => empty( $errors ), 'schema' => self::HANDOFF_SCHEMA, 'destination' => sanitize_key( isset( $payload['destination'] ) ? $payload['destination'] : '' ), 'errors' => $errors, 'warnings' => empty( $payload['evidence'] ) ? array( 'No verified source records are attached to this handoff.' ) : array() );
    }
    ...
    public static function rest_prepare( WP_REST_Request $request ) {
        ...
        if ( strlen( trim( $question ) ) < 3 ) {
            return new WP_Error( 'sc_rl_v660_empty_question', 'Enter a research question before preparing a handoff.', array( 'status' => 400 ) );
        }
        ...
        if ( is_wp_error( $result ) ) {
            ...
            $result = array( 'ok' => true, 'version' => self::VERSION, 'handoff' => $handoff, 'fallback' => true, 'backend_error' => $result->get_error_message() );
        }
    ...
    public static function rest_artifact_return( WP_REST_Request $request ) {
        ...
        if ( empty( $params['artifact_id'] ) || empty( $params['handoff_id'] ) || empty( $params['destination'] ) || empty( $params['artifact_type'] ) ) {
            return new WP_Error( 'sc_rl_v660_invalid_artifact', 'Artifact ID, handoff ID, destination, and artifact type are required.', array( 'status' => 400 ) );
        }
        ...
        array_unshift( $returns, array( 'artifact_id' => $params['artifact_id'], 'handoff_id' => $params['handoff_id'], 'destination' => $params['destination'], 'artifact_type' => $params['artifact_type'], 'created_utc' => isset( $params['created_utc'] ) ? $params['created_utc'] : gmdate( 'c' ) ) );
    ...
    public static function rest_export() {
        return new WP_REST_Response( array(
            ...
            'schemas' => array( 'handoff' => self::HANDOFF_SCHEMA, 'route' => self::ROUTE_SCHEMA, 'artifact_return' => self::ARTIFACT_SCHEMA ),
            'capabilities' => self::capabilities(),
    ...
    public static function render_admin_page() {
        ...
        <div class="wrap">
            ...
            <p>v6.6.0 exposes versioned, validated research handoffs for Workbench, Decision Studio, Site Intelligence, Lab, and Feature Suggestions. Disabling a destination removes it from public actions and handoff preparation.</p>
            <form method="post">
                <?php wp_nonce_field( 'sc_rl_v660_save_action' ); ?>
                <table class="widefat striped"><thead><tr><th>Destination</th><th>Available</th><th>URL</th><th>Version</th><th>Contract</th></tr></thead><tbody>
