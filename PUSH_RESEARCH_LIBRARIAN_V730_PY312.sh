...
CANONICAL_ZIP_NAME="sustainable-catalyst-research-librarian-ai-v7.3.0-repository.zip"
ZIP_PATH="${1:-$SCRIPT_DIR/$CANONICAL_ZIP_NAME}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v730-clean}"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v730.XXXXXX")"
...
[[ -f "$SOURCE_DIR/backend/app/evidence_quality.py" ]] || fail "The v7.3.0 evidence-quality backend module is missing."
[[ -f "$SOURCE_DIR/docs/V730_SOURCE_EVALUATION_EVIDENCE_COMPARISON_RESEARCH_QUALITY_SIGNALS.md" ]] || fail "The v7.3.0 release documentation is missing."
[[ -f "$SOURCE_DIR/data/research_librarian_source_evaluation_manifest_v7.3.0.json" ]] || fail "The v7.3.0 source evaluation manifest is missing."
[[ -f "$SOURCE_DIR/tests/v730-source-evaluation-evidence-quality-contract-test.php" ]] || fail "The v7.3.0 WordPress contract test is missing."
[[ -f "$SOURCE_DIR/backend/tests/test_v730_source_evaluation.py" ]] || fail "The v7.3.0 backend test suite is missing."
grep -q 'Version: 7.3.0' "$SOURCE_DIR/sustainable-catalyst-research-librarian-ai.php" || fail "The plugin version marker is not 7.3.0."
grep -q '__version__ = "7.3.0"' "$SOURCE_DIR/backend/app/__init__.py" || fail "The backend version marker is not 7.3.0."
...
grep -q 'sc-source-evaluation/1.0' "$SOURCE_DIR/backend/app/evidence_quality.py" || fail "The v7.3.0 source evaluation contract is missing."
grep -q 'sc-evidence-comparison/1.0' "$SOURCE_DIR/backend/app/evidence_quality.py" || fail "The v7.3.0 evidence comparison contract is missing."
grep -q 'sc-evidence-gap-report/1.0' "$SOURCE_DIR/backend/app/evidence_quality.py" || fail "The v7.3.0 evidence-gap contract is missing."
grep -q 'sc-connected-research-api/1.2' "$SOURCE_DIR/backend/app/platform_v7.py" || fail "Connected Research API 1.2 is missing."
grep -q 'sc-research-librarian-public-workspace/2.2' "$SOURCE_DIR/backend/app/main.py" || fail "Public workspace contract 2.2 is missing."
grep -q 'data-sc-rl-v730-quality-run' "$SOURCE_DIR/includes/class-sc-rl-v700-connected-platform.php" || fail "The evidence-quality workspace action is missing."
...
bash -n PUSH_RESEARCH_LIBRARIAN_V730_PY312.sh
git add -A
if git diff --cached --quiet; then fail "No v7.3.0 changes were detected after extraction."; fi
git commit -m "Build Research Librarian v${RELEASE_VERSION}"
git tag -a "$RELEASE_TAG" -m "Research Librarian v${RELEASE_VERSION} — Source Evaluation, Evidence Comparison & Research Quality Signals"
git push origin main
git push origin "$RELEASE_TAG"

printf '\nResearch Librarian v%s validated and pushed successfully.\n' "$RELEASE_VERSION"
printf 'Render should auto-deploy main. Then update WordPress with the v7.3.0 WordPress package and verify Evaluate context against a saved Library/project context.\n'
