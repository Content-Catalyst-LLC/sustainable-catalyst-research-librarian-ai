...
ZIP_PATH="${1:-$SCRIPT_DIR/sustainable-catalyst-research-librarian-ai-v8.3.0-repository.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v830-clean}"
fail(){ printf 'ERROR: %s\n' "$*" >&2; exit 1; }
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v830.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
...
for f in backend/app/async_jobs.py backend/app/api/jobs.py backend/app/contracts/jobs.py backend/app/services/document_jobs.py backend/app/workers/document_worker.py backend/migrations/004_async_document_processing_runtime.sql backend/tests/test_v830_async_document_runtime.py docs/V830_ASYNCHRONOUS_INGESTION_DOCUMENT_PROCESSING_RUNTIME.md data/research_librarian_async_runtime_manifest_v8.3.0.json tests/v830-asynchronous-ingestion-document-processing-contract-test.php deploy/contabo/upgrade_research_librarian_backend_v8_3_0_contabo.sh; do [[ -f "$SRC/$f" ]]||fail "v8.3.0 file missing: $f"; done
grep -q 'Version: 8.3.0' "$SRC/sustainable-catalyst-research-librarian-ai.php"||fail "Plugin version mismatch."
grep -q '__version__ = "8.3.0"' "$SRC/backend/app/__init__.py"||fail "Backend version mismatch."
grep -q 'FOR UPDATE SKIP LOCKED' "$SRC/backend/app/async_jobs.py"||fail "Postgres durable queue claim contract missing."
grep -q '/opt/sustainable-catalyst/research-librarian-ai' "$SRC/deploy/contabo/upgrade_research_librarian_backend_v8_3_0_contabo.sh"||fail "Correct VPS Research Librarian root missing."
...
bash -n deploy/contabo/upgrade_research_librarian_backend_v8_3_0_contabo.sh
...
git add -A; git diff --cached --quiet&&fail "No v8.3.0 changes detected."
git commit -m "Build Research Librarian v${VERSION} — asynchronous ingestion runtime"
git tag -a "$TAG" -m "Research Librarian v${VERSION} — Asynchronous Ingestion & Document Processing Runtime"
git push origin main; git push origin "$TAG"
printf '\nResearch Librarian v%s validated and pushed successfully.\n' "$VERSION"
printf 'Deploy the backend package to Contabo before installing the WordPress v8.3.0 package.\n'
