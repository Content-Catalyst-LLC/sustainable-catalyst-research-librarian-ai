...
SCRIPT_REVISION="v660-py312-platform-handoffs-r1"
REPO_URL="git@github.com:Content-Catalyst-LLC/sustainable-catalyst-research-librarian-ai.git"
ZIP_PATH="${1:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-v6.6.0.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v660-clean}"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v660.XXXXXX")"
...
grep -q 'Version: 6.6.0' "$SOURCE_DIR/sustainable-catalyst-research-librarian-ai.php" || fail "The package version marker is not 6.6.0."
...
grep -q 'SCHEMA_VERSION = 7' "$SOURCE_DIR/backend/app/store.py" || fail "The package does not declare SQLite schema version 7."
...
grep -q 'sc-research-handoff/2.0' "$SOURCE_DIR/backend/app/platform_handoffs.py" || fail "The v6.6.0 handoff contract is missing."
grep -q 'sc-research-route/2.0' "$SOURCE_DIR/backend/app/platform_handoffs.py" || fail "The v6.6.0 route contract is missing."
...
git tag -a "$RELEASE_TAG" -m "Research Librarian v${RELEASE_VERSION} — Platform Intelligence and Typed Research Handoffs"
