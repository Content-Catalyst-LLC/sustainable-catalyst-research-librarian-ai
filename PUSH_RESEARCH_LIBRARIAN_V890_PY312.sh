...
ZIP_PATH="${1:-$SCRIPT_DIR/sustainable-catalyst-research-librarian-ai-v8.9.0-repository.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v890-clean}"
fail(){ printf 'ERROR: %s\n' "$*" >&2; exit 1; }
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v890.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
...
for f in backend/app/source_identity.py backend/app/api/sources.py backend/app/contracts/sources.py backend/tests/test_v860_source_identity.py docs/V860_SOURCE_IDENTITY_DEDUPLICATION_CITATION_GRAPH.md data/research_librarian_source_identity_manifest_v8.6.0.json tests/v860-source-identity-citation-graph-contract-test.php backend/migrations/005_source_identity_citation_graph.sql RELEASE_MANIFEST_V860.json backend/app/contracts/evidence_bridge.py backend/app/services/core_evidence_bridge.py backend/tests/test_v870_core_evidence_bridge.py docs/V870_CORE_EVIDENCE_BRIDGE.md data/research_librarian_core_evidence_bridge_manifest_v8.7.0.json tests/v870-core-evidence-bridge-contract-test.php RELEASE_MANIFEST_V870.json backend/app/document_intelligence.py backend/app/api/documents.py backend/app/contracts/documents.py backend/tests/test_v850_document_intelligence.py docs/V850_DOCUMENT_INTELLIGENCE_SCHOLARLY_PARSING.md data/research_librarian_document_intelligence_manifest_v8.5.0.json tests/v850-document-intelligence-scholarly-parsing-contract-test.php backend/app/advanced_retrieval.py backend/tests/test_v840_advanced_retrieval.py backend/app/async_jobs.py backend/app/clients/platform_core.py backend/migrations/004_async_document_processing_runtime.sql deploy/contabo/upgrade_research_librarian_backend_v8_7_0_contabo.sh backend/app/contracts/research_sync.py backend/app/services/core_research_sync.py backend/tests/test_v880_core_research_object_sync.py docs/V880_CORE_RESEARCH_OBJECT_SYNCHRONIZATION.md data/research_librarian_core_research_sync_manifest_v8.8.0.json tests/v880-core-research-object-synchronization-contract-test.php RELEASE_MANIFEST_V880.json deploy/contabo/upgrade_research_librarian_backend_v8_8_0_contabo.sh backend/app/contracts/research_intelligence_extraction.py backend/app/services/research_intelligence_extraction.py backend/tests/test_v890_finding_claim_evidence_extraction.py docs/V890_FINDING_CLAIM_EVIDENCE_EXTRACTION_PIPELINE.md data/research_librarian_finding_claim_evidence_extraction_manifest_v8.9.0.json tests/v890-finding-claim-evidence-extraction-contract-test.php RELEASE_MANIFEST_V890.json deploy/contabo/upgrade_research_librarian_backend_v8_9_0_contabo.sh; do [[ -f "$SRC/$f" ]]||fail "v8.9.0 required file missing: $f"; done
grep -q 'Version: 8.9.0' "$SRC/sustainable-catalyst-research-librarian-ai.php"||fail "Plugin version mismatch."
grep -q '__version__ = "8.9.0"' "$SRC/backend/app/__init__.py"||fail "Backend version mismatch."
...
grep -q 'CORE_EVIDENCE_BRIDGE_SCHEMA' "$SRC/backend/app/contracts/evidence_bridge.py"||fail "v8.7 Core Evidence Bridge contract missing."
grep -q 'CORE_RESEARCH_SYNC_SCHEMA' "$SRC/backend/app/contracts/research_sync.py"||fail "v8.8 research sync contract missing."
grep -q 'def build_project_sync_plan' "$SRC/backend/app/services/core_research_sync.py"||fail "v8.8 project sync planner missing."
grep -q 'def synchronize_project_research_objects' "$SRC/backend/app/services/core_research_sync.py"||fail "v8.8 project synchronization missing."
grep -q '/research-sync/synchronize' "$SRC/backend/app/api/core.py"||fail "v8.8 research synchronization API missing."
grep -q '/v1/research/project-state/states' "$SRC/backend/app/clients/platform_core.py"||fail "Platform Core project-state client missing."
grep -q '8.8.0' "$SRC/data/research_librarian_core_research_sync_manifest_v8.8.0.json"||fail "v8.8 research sync manifest missing release identity."

grep -q 'RESEARCH_INTELLIGENCE_EXTRACTION_SCHEMA' "$SRC/backend/app/contracts/research_intelligence_extraction.py"||fail "v8.9 extraction contract missing."
grep -q 'def extract_candidates' "$SRC/backend/app/services/research_intelligence_extraction.py"||fail "v8.9 deterministic candidate extraction missing."
grep -q 'def promote_candidate' "$SRC/backend/app/services/research_intelligence_extraction.py"||fail "v8.9 Core candidate promotion missing."
grep -q '/research-intelligence/extract' "$SRC/backend/app/api/core.py"||fail "v8.9 extraction API missing."
grep -q '/research-intelligence/promote' "$SRC/backend/app/api/core.py"||fail "v8.9 promotion API missing."
grep -q '"research-intelligence-extraction"' "$SRC/backend/app/async_jobs.py"||fail "v8.9 durable extraction job type missing."
grep -q '"project_state": "/v1/research/project-state/readiness"' "$SRC/backend/app/clients/platform_core.py"||fail "v8.9 project-state readiness registration fix missing."
grep -q '"finding_claim_evidence_intelligence": "/v1/research/intelligence/readiness"' "$SRC/backend/app/clients/platform_core.py"||fail "v8.9 Core research-intelligence readiness registration missing."
grep -q '8.9.0' "$SRC/data/research_librarian_finding_claim_evidence_extraction_manifest_v8.9.0.json"||fail "v8.9 extraction manifest missing release identity."

grep -q 'def promote_source_snapshot' "$SRC/backend/app/services/core_evidence_bridge.py"||fail "v8.7 source snapshot promotion missing."
grep -q 'def promote_passage_evidence' "$SRC/backend/app/services/core_evidence_bridge.py"||fail "v8.7 passage evidence promotion missing."
grep -q '/evidence/source-snapshots/promote' "$SRC/backend/app/api/core.py"||fail "v8.7 snapshot promotion API missing."
grep -q '/evidence/passages/promote' "$SRC/backend/app/api/core.py"||fail "v8.7 passage promotion API missing."
grep -q '/v1/source-snapshots' "$SRC/backend/app/clients/platform_core.py"||fail "Core SourceSnapshot client contract missing."
grep -q '/v1/evidence-records' "$SRC/backend/app/clients/platform_core.py"||fail "Core EvidenceRecord client contract missing."
...
grep -q '/opt/sustainable-catalyst/research-librarian-ai' "$SRC/deploy/contabo/upgrade_research_librarian_backend_v8_7_0_contabo.sh"||fail "Correct VPS Research Librarian root missing."
...
bash -n deploy/contabo/upgrade_research_librarian_backend_v8_7_0_contabo.sh
bash -n deploy/contabo/upgrade_research_librarian_backend_v8_8_0_contabo.sh
bash -n deploy/contabo/upgrade_research_librarian_backend_v8_9_0_contabo.sh
...
git add -A; git diff --cached --quiet&&fail "No v8.9.0 changes detected."
git commit -m "Build Research Librarian v${VERSION} — Finding, Claim & Evidence Extraction Pipeline"
git tag -a "$TAG" -m "Research Librarian v${VERSION} — Finding, Claim & Evidence Extraction Pipeline"
git push origin main; git push origin "$TAG"
printf '\nResearch Librarian v%s validated and pushed successfully.\n' "$VERSION"
printf 'Deploy the backend package to Contabo before installing the WordPress v8.9.0 package.\n'
