...
ZIP_PATH="${1:-$SCRIPT_DIR/sustainable-catalyst-research-librarian-ai-v8.12.0-repository.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v8120-clean}"
fail(){ printf 'ERROR: %s\n' "$*" >&2; exit 1; }
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v8120.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
...
for f in \
 backend/app/contracts/visual_research.py \
 backend/app/services/visual_research.py \
 backend/tests/test_v8120_visual_research.py \
 docs/V8120_VISUAL_RESEARCH_INTELLIGENCE.md \
 data/research_librarian_visual_research_manifest_v8.12.0.json \
 tests/v8120-visual-research-intelligence-contract-test.php \
 RELEASE_MANIFEST_V8120.json \
 deploy/contabo/upgrade_research_librarian_backend_v8_12_0_contabo.sh; do
  [[ -f "$SRC/$f" ]]||fail "v8.12.0 required file missing: $f"
done
grep -q 'Version: 8.12.0' "$SRC/sustainable-catalyst-research-librarian-ai.php"||fail "Plugin version mismatch."
grep -q '__version__ = "8.12.0"' "$SRC/backend/app/__init__.py"||fail "Backend version mismatch."
grep -q 'VISUAL_RESEARCH_SCHEMA' "$SRC/backend/app/contracts/visual_research.py"||fail "v8.12 visual research contract missing."
grep -q 'def build_plan' "$SRC/backend/app/services/visual_research.py"||fail "v8.12 visual planner missing."
grep -q 'promote_plan' "$SRC/backend/app/services/visual_research.py"||fail "v8.12 Core visual promotion missing."
grep -q '/visual-research/promote' "$SRC/backend/app/api/core.py"||fail "v8.12 visual promotion API missing."
grep -q '"visual-research-plan"' "$SRC/backend/app/async_jobs.py"||fail "v8.12 durable visual planning job missing."
grep -q '/v1/visual-reasoning/objects' "$SRC/backend/app/clients/platform_core.py"||fail "Platform Core visual client missing."
grep -q '/v1/research/unified-runtime/visual-bindings' "$SRC/backend/app/clients/platform_core.py"||fail "Platform Core unified visual binding client missing."
...
echo "=== VALIDATING Research Librarian v8.12.0 ==="
php_count=0
...
git add -A
if git diff --cached --quiet; then fail "No v8.12.0 changes detected after applying release."; fi
git commit -m "Build Research Librarian v8.12.0 — visual research intelligence"
git push origin main
if git rev-parse "$TAG" >/dev/null 2>&1; then fail "Tag $TAG already exists locally."; fi
git tag -a "$TAG" -m "Research Librarian v8.12.0 — Visual Research Intelligence"
git push origin "$TAG"
echo "Research Librarian v8.12.0 validated and pushed successfully."
echo "Deploy the backend package to Contabo before installing the WordPress v8.12.0 package."
