...
SCRIPT_REVISION="v631-py312-cold-start-hardening-r1"
REPO_URL="git@github.com:Content-Catalyst-LLC/sustainable-catalyst-research-librarian-ai.git"
ZIP_PATH="${1:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-v6.3.1.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v631-clean}"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v631.XXXXXX")"
...
[[ -f "$SOURCE_DIR/tests/cold-start-recovery-hardening-contract-test.php" ]] || fail "The v6.3.1 hardening contract test is missing."
[[ -f "$SOURCE_DIR/data/research_librarian_cold_start_recovery_manifest_v6.3.1.json" ]] || fail "The v6.3.1 release manifest is missing."
[[ -f "$SOURCE_DIR/pytest.ini" ]] || fail "The package does not contain repository-root pytest configuration."
grep -q 'Version: 6.3.1' "$SOURCE_DIR/sustainable-catalyst-research-librarian-ai.php" || fail "The package version marker is not 6.3.1."
grep -q 'SCHEMA_VERSION = 4' "$SOURCE_DIR/backend/app/store.py" || fail "The package does not declare SQLite schema version 4."
...
git tag -a "$RELEASE_TAG" -m "Research Librarian v${RELEASE_VERSION} — Cold-Start and Recovery Hardening"
