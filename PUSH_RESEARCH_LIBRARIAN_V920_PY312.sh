...
ZIP_PATH="${1:-$SCRIPT_DIR/sustainable-catalyst-research-librarian-ai-v9.2.0-repository.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v920-clean}"
fail(){ printf 'ERROR: %s\n' "$*" >&2; exit 1; }
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v920.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
...
for f in \
 ...
 deploy/contabo/upgrade_research_librarian_backend_v9_2_0_contabo.sh; do
  [[ -f "$SRC/$f" ]]||fail "v9.1.0 required file missing: $f"
done
grep -q 'Version: 9.2.0' "$SRC/sustainable-catalyst-research-librarian-ai.php"||fail "Plugin version mismatch."
grep -q '__version__ = "9.2.0"' "$SRC/backend/app/__init__.py"||fail "Backend version mismatch."
...
echo "=== VALIDATING Research Librarian v9.2.0 ==="
php_count=0
...
git add -A
if git diff --cached --quiet; then fail "No v9.2.0 changes detected after applying release."; fi
git commit -m "Build Research Librarian v9.2.0 — original research scholarly research environment"
git push origin main
if git rev-parse "$TAG" >/dev/null 2>&1; then fail "Tag $TAG already exists locally."; fi
git tag -a "$TAG" -m "Research Librarian v9.2.0 — Original Research & Scholarly Research Environment"
git push origin "$TAG"
echo "Research Librarian v9.2.0 validated and pushed successfully."
echo "Deploy the backend package to Contabo before installing the WordPress v9.2.0 package."
