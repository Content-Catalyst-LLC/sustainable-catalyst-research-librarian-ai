...
VERSION="10.4.0"; TAG="v${VERSION}"
REPO_URL="git@github.com:Content-Catalyst-LLC/sustainable-catalyst-research-librarian-ai.git"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ZIP_PATH="${1:-$SCRIPT_DIR/sustainable-catalyst-research-librarian-ai-v10.4.0-repository.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v1040-clean}"
fail(){ printf 'ERROR: %s\n' "$*" >&2; exit 1; }
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v1040.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
...
for f in \
 ...
 deploy/contabo/upgrade_research_librarian_backend_v10_2_0_contabo.sh \
 backend/app/contracts/evidence_search_strategy.py backend/app/services/evidence_search_strategy.py backend/tests/test_v1030_evidence_search_strategy.py backend/migrations/018_evidence_search_strategy_engine.sql \
 docs/V1030_EVIDENCE_SEARCH_STRATEGY_ENGINE.md data/research_librarian_evidence_search_strategy_manifest_v10.3.0.json tests/v1030-evidence-search-strategy-engine-contract-test.php RELEASE_MANIFEST_V1030.json \
 deploy/contabo/upgrade_research_librarian_backend_v10_3_0_contabo.sh \
 ...
 docs/V1040_SYSTEMATIC_REVIEW_EVIDENCE_SYNTHESIS_INTELLIGENCE.md data/research_librarian_systematic_review_evidence_synthesis_manifest_v10.4.0.json tests/v1040-systematic-review-evidence-synthesis-contract-test.php RELEASE_MANIFEST_V1040.json \
 deploy/contabo/upgrade_research_librarian_backend_v10_4_0_contabo.sh; do [[ -f "$SRC/$f" ]]||fail "required file missing: $f"; done
grep -q 'Version: 10.4.0' "$SRC/sustainable-catalyst-research-librarian-ai.php"||fail "Plugin version mismatch."
grep -q '__version__ = "10.4.0"' "$SRC/backend/app/__init__.py"||fail "Backend version mismatch."
...
grep -q 'EVIDENCE_SEARCH_STRATEGY_SCHEMA' "$SRC/backend/app/contracts/evidence_search_strategy.py"||fail "v10.3 evidence-search contract missing."
grep -q '/evidence-search-strategy/strategies/{strategy_id}/coverage' "$SRC/backend/app/api/core.py"||fail "v10.3 evidence-search coverage API missing."
grep -q '"evidence-search-strategy-snapshot"' "$SRC/backend/app/async_jobs.py"||fail "v10.3 durable evidence-search snapshot job missing."
grep -q 'human_search_protocol_approval_required.*True' "$SRC/backend/app/services/evidence_search_strategy.py"||fail "v10.3 human protocol guardrail missing."
grep -q 'automatic_external_search_execution.*False' "$SRC/backend/app/services/evidence_search_strategy.py"||fail "v10.3 automatic-search guardrail missing."
grep -q 'automatic_source_acceptance.*False' "$SRC/backend/app/services/evidence_search_strategy.py"||fail "v10.3 source-acceptance guardrail missing."
grep -q 'evidence-search-strategy-plan' "$SRC/backend/app/services/unified_scholarly_ai_environment.py"||fail "v10.3 unified environment search-strategy binding missing."

grep -q 'SYSTEMATIC_REVIEW_EVIDENCE_SYNTHESIS_SCHEMA' "$SRC/backend/app/contracts/systematic_review_evidence_synthesis.py"||fail "v10.4 systematic-review contract missing."
grep -q '/systematic-review-evidence-synthesis/reviews/{review_id}/extraction-matrix' "$SRC/backend/app/api/core.py"||fail "v10.4 extraction-matrix API missing."
grep -q '"systematic-review-evidence-synthesis-snapshot"' "$SRC/backend/app/async_jobs.py"||fail "v10.4 durable systematic-review snapshot job missing."
grep -q 'human_screening_decisions_required.*True' "$SRC/backend/app/services/systematic_review_evidence_synthesis.py"||fail "v10.4 human-screening guardrail missing."
grep -q 'automatic_study_inclusion.*False' "$SRC/backend/app/services/systematic_review_evidence_synthesis.py"||fail "v10.4 automatic-study-inclusion guardrail missing."
grep -q 'automatic_meta_analysis_execution.*False' "$SRC/backend/app/services/systematic_review_evidence_synthesis.py"||fail "v10.4 automatic-meta-analysis guardrail missing."
grep -q 'systematic-review-plan' "$SRC/backend/app/services/unified_scholarly_ai_environment.py"||fail "v10.4 unified environment review binding missing."

printf '=== VALIDATING Research Librarian v10.4.0 ===\n'
...
printf 'WordPress current-release contract/functional tests passed (legacy live-ai-provider identity test excluded).\n'
...
git add -A
git commit -m "Build Research Librarian v10.4.0 — Systematic Review & Evidence Synthesis Intelligence"
git push origin main
if git rev-parse "$TAG" >/dev/null 2>&1; then fail "Tag $TAG already exists locally."; fi
git tag -a "$TAG" -m "Research Librarian v10.4.0 — Systematic Review & Evidence Synthesis Intelligence"
git push origin "$TAG"
printf 'Research Librarian v10.4.0 validated and pushed successfully.\nDeploy the backend package to Contabo before installing the WordPress v10.4.0 package.\n'
