<?php
/** Static release contract for Research Librarian AI v7.2.0 Library object/context alignment. */
...
$checks = array(
    ...
    'sqlite_schema_13' => false !== strpos( $store, 'SCHEMA_VERSION = 13' ),
    ...
    'api_schema_11' => false !== strpos( $backend, 'sc-connected-research-api/1.1' ) || false !== strpos( file_get_contents( $root . '/backend/app/platform_v7.py' ), 'sc-connected-research-api/1.1' ),
    'workspace_schema_21' => false !== strpos( $backend, 'sc-research-librarian-public-workspace/2.1' ),
...
echo json_encode( array( 'version' => '7.2.0', 'checks' => $checks, 'passed' => count( $checks ) - count( $failed ), 'failed' => count( $failed ), 'failures' => $failed ), JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) . PHP_EOL;
exit( $failed ? 1 : 0 );
