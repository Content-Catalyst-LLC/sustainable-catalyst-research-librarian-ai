...
$checks = array(
    ...
    'compose_release' => false !== strpos($compose, '8.3.0'),
);
$failed = array_keys(array_filter($checks, fn($ok) => !$ok));
if ($failed) {
    fwrite(STDERR, 'Failed v8.3.0 contract checks: ' . implode(', ', $failed) . PHP_EOL);
    exit(1);
}
echo "Research Librarian v8.3.0 asynchronous ingestion/document-processing contract passed.\n";
