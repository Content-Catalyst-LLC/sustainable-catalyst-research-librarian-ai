...
$checks = array(
    ...
    'version_constant' => false !== strpos( $module, "const VERSION = '7.0.5';" ),
    ...
    'persistent_state' => false !== strpos( $module, "sc_rl_v703_index_build_state" ) && false !== strpos( $module, "sc-rl-async-index-build/7.0.5" ),
    ...
    'transactional_batches' => false !== strpos( $module, 'wordpress-async-full-sync-v7.0.5' ) && false !== strpos( $module, '\'batch_count\' => $batch_count' ),
    ...
    'embedding_after_verify' => false !== strpos( $module, "'starting-embeddings'" ) && false !== strpos( $module, 'v7.0.5-async-index-build' ),
    ...
    'manifest' => is_array( $manifest ) && '7.0.5' === ( $manifest['version'] ?? '' ) && ! empty( $manifest['capabilities']['transactional_index_swap'] ) && false === ( $manifest['paid_infrastructure_required'] ?? true ),
...
echo json_encode( array( 'version' => '7.0.5', 'checks' => $checks, 'passed' => count( $checks ) - count( $failed ), 'failed' => count( $failed ), 'failures' => $failed ), JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) . PHP_EOL;
exit( $failed ? 1 : 0 );
