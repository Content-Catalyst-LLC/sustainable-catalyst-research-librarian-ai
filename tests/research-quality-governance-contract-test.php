<?php
/** Static release contract checks for Research Librarian AI v6.7.0. */
...
$manifest=json_decode(file_get_contents($root.'/data/research_librarian_quality_governance_manifest_v6.7.0.json'),true);
$checks=array(
 ...
 'schema_nine'=>false!==strpos($store,'SCHEMA_VERSION = 9'),
 'index_nine'=>false!==strpos($store,'sc-research-librarian-knowledge-index/9.0'),
 ...
 'manifest_version'=>is_array($manifest)&&'6.7.0'===($manifest['version']??''),
 'docs'=>false!==strpos($docs,'Research Quality and Governance Center'),
);
$failed=array_keys(array_filter($checks,function($ok){return !$ok;}));
echo json_encode(array('version'=>'6.7.0','checks'=>$checks,'passed'=>count($checks)-count($failed),'failed'=>count($failed),'failures'=>$failed),JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES).PHP_EOL;
exit($failed?1:0);
