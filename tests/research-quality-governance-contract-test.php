<?php
/** Static release contract checks for Research Librarian AI v7.0.1. */
...
$manifest=json_decode(file_get_contents($root.'/data/research_librarian_quality_governance_manifest_v7.0.1.json'),true);
$checks=array(
...
echo json_encode(array('version'=>'7.0.1','checks'=>$checks,'passed'=>count($checks)-count($failed),'failed'=>count($failed),'failures'=>$failed),JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES).PHP_EOL;
exit($failed?1:0);
