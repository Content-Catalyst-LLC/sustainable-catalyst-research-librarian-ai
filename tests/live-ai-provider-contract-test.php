<?php
/**
 * Static release contract checks for Research Librarian AI v6.2.1.
 *
...
$assert( 'plugin_version_621', 1 === preg_match( "/const\s+VERSION\s*=\s*'6\.2\.1';/", $main ) );
...
$v621 = file_get_contents( $root . '/includes/class-sc-rl-v621-endpoint-reliability.php' );
$backend = file_get_contents( $root . '/backend/app/main.py' );
$retrieval = file_get_contents( $root . '/backend/app/retrieval.py' );
$assert( 'python_intelligence_module', false !== strpos( $v621, 'SC_RL6_V621_Endpoint_Reliability' ) );
$assert( 'python_admin_submenu', false !== strpos( $v621, "'sc-rl-python-intelligence'" ) );
$assert( 'full_library_limit', false !== strpos( $v621, "'max_records' => 5000" ) );
