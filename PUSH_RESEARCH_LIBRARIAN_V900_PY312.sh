...
ZIP_PATH="${1:-$SCRIPT_DIR/sustainable-catalyst-research-librarian-ai-v9.0.0-repository.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v900-clean}"
fail(){ printf 'ERROR: %s\n' "$*" >&2; exit 1; }
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v900.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
...
for f in \
 ...
 docs/V900_UNIFIED_RESEARCH_INTELLIGENCE_RUNTIME.md \
 data/research_librarian_unified_research_runtime_manifest_v9.0.0.json \
 tests/v900-unified-research-intelligence-runtime-contract-test.php \
 RELEASE_MANIFEST_V900.json \
 deploy/contabo/upgrade_research_librarian_backend_v9_0_0_contabo.sh; do
  [[ -f "$SRC/$f" ]]||fail "v9.0.0 required file missing: $f"
done
grep -q 'Version: 9.0.0' "$SRC/sustainable-catalyst-research-librarian-ai.php"||fail "Plugin version mismatch."
grep -q '__version__ = "9.0.0"' "$SRC/backend/app/__init__.py"||fail "Backend version mismatch."
...
echo "=== VALIDATING Research Librarian v9.0.0 ==="
php_count=0
...
git add -A
if git diff --cached --quiet; then fail "No v9.0.0 changes detected after applying release."; fi
git commit -m "Build Research Librarian v9.0.0 — unified research intelligence runtime"
git push origin main
if git rev-parse "$TAG" >/dev/null 2>&1; then fail "Tag $TAG already exists locally."; fi
git tag -a "$TAG" -m "Research Librarian v9.0.0 — Unified Research Intelligence Runtime"
git push origin "$TAG"
echo "Research Librarian v9.0.0 validated and pushed successfully."
echo "Deploy the backend package to Contabo before installing the WordPress v9.0.0 package."
