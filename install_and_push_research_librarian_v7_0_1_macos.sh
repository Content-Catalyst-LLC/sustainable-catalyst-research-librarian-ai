...
SCRIPT_REVISION="v701-py312-canonical-index-repair-r1"
REPO_URL="git@github.com:Content-Catalyst-LLC/sustainable-catalyst-research-librarian-ai.git"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ZIP_PATH="${1:-$SCRIPT_DIR/sustainable-catalyst-research-librarian-ai-v7.0.1.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v701-clean}"
...
CHECKSUM_FILE="$(dirname "$ZIP_PATH")/SHA256SUMS"
if [[ -f "$CHECKSUM_FILE" ]]; then
  command -v shasum >/dev/null 2>&1 || fail "shasum is required for checksum verification."
  ZIP_NAME="$(basename "$ZIP_PATH")"
  grep -F "  $ZIP_NAME" "$CHECKSUM_FILE" >/dev/null 2>&1 || fail "SHA256SUMS has no entry for $ZIP_NAME."
  ( cd "$(dirname "$ZIP_PATH")" && grep -F "  $ZIP_NAME" SHA256SUMS | shasum -a 256 -c - ) || fail "Release ZIP checksum verification failed."
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v701.XXXXXX")"
...
[[ -f "$SOURCE_DIR/data/research_librarian_quality_governance_manifest_v7.0.1.json" ]] || fail "The inherited governance manifest is missing."
...
[[ -f "$SOURCE_DIR/data/research_librarian_connected_platform_manifest_v7.0.1.json" ]] || fail "The v7 connected platform manifest is missing."
[[ -f "$SOURCE_DIR/docs/V700_CONNECTED_RESEARCH_INTELLIGENCE_PLATFORM.md" ]] || fail "The v7 release documentation is missing."
[[ -f "$SOURCE_DIR/docs/V701_CANONICAL_INDEX_CREDENTIAL_EMBEDDING_REPAIR.md" ]] || fail "The v7.0.1 repair documentation is missing."
[[ -f "$SOURCE_DIR/data/research_librarian_v701_repair_manifest.json" ]] || fail "The v7.0.1 repair manifest is missing."
[[ -f "$SOURCE_DIR/tests/v701-canonical-index-repair-contract-test.php" ]] || fail "The v7.0.1 repair contract test is missing."
[[ -f "$SOURCE_DIR/backend/tests/test_v701_repair.py" ]] || fail "The v7.0.1 backend repair tests are missing."
[[ -f "$SOURCE_DIR/pytest.ini" ]] || fail "The package does not contain repository-root pytest configuration."
grep -q 'Version: 7.0.1' "$SOURCE_DIR/sustainable-catalyst-research-librarian-ai.php" || fail "The package version marker is not 7.0.1."
...
grep -q "get_post_types( array( 'public' => true ), 'names' )" "$SOURCE_DIR/sustainable-catalyst-research-librarian-ai.php" || fail "All-public-post-type fallback indexing is missing."
grep -q 'Full Sync and Complete Embedding Queue' "$SOURCE_DIR/includes/class-sc-rl-v630-durable-index.php" || fail "The one-click canonical repair workflow is missing."
grep -q 'sc_rl_v701_embedding_queue_event' "$SOURCE_DIR/includes/class-sc-rl-v630-durable-index.php" || fail "The resumable v7.0.1 embedding queue is missing."
grep -q '@app.get("/v1/provider/diagnostics"' "$SOURCE_DIR/backend/app/main.py" || fail "The provider diagnostics endpoint is missing."
grep -q '@app.post("/v1/knowledge/embeddings/test"' "$SOURCE_DIR/backend/app/main.py" || fail "The embedding connection test endpoint is missing."
grep -q 'credential_source.*SC_RL_GEMINI_API_KEY' "$SOURCE_DIR/backend/app/provider.py" || fail "The canonical Gemini credential source is not declared."
grep -q 'embedding_model=?' "$SOURCE_DIR/backend/app/store.py" || fail "Current-model semantic coverage filtering is missing."
...
git tag -a "$RELEASE_TAG" -m "Research Librarian v${RELEASE_VERSION} — Canonical Index, Credential, and Embedding Queue Repair"
