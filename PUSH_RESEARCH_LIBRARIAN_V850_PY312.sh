...
ZIP_PATH="${1:-$SCRIPT_DIR/sustainable-catalyst-research-librarian-ai-v8.5.0-repository.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v850-clean}"
fail(){ printf 'ERROR: %s\n' "$*" >&2; exit 1; }
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v850.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
...
for f in backend/app/document_intelligence.py backend/app/api/documents.py backend/app/contracts/documents.py backend/tests/test_v850_document_intelligence.py docs/V850_DOCUMENT_INTELLIGENCE_SCHOLARLY_PARSING.md data/research_librarian_document_intelligence_manifest_v8.5.0.json tests/v850-document-intelligence-scholarly-parsing-contract-test.php backend/app/advanced_retrieval.py backend/tests/test_v840_advanced_retrieval.py backend/app/async_jobs.py backend/app/clients/platform_core.py backend/migrations/004_async_document_processing_runtime.sql deploy/contabo/upgrade_research_librarian_backend_v8_5_0_contabo.sh; do [[ -f "$SRC/$f" ]]||fail "v8.5.0 file missing: $f"; done
grep -q 'Version: 8.5.0' "$SRC/sustainable-catalyst-research-librarian-ai.php"||fail "Plugin version mismatch."
grep -q '__version__ = "8.5.0"' "$SRC/backend/app/__init__.py"||fail "Backend version mismatch."
...
grep -q '/opt/sustainable-catalyst/research-librarian-ai' "$SRC/deploy/contabo/upgrade_research_librarian_backend_v8_5_0_contabo.sh"||fail "Correct VPS Research Librarian root missing."
...
bash -n deploy/contabo/upgrade_research_librarian_backend_v8_5_0_contabo.sh
...
git add -A; git diff --cached --quiet&&fail "No v8.5.0 changes detected."
git commit -m "Build Research Librarian v${VERSION} — document intelligence and scholarly parsing"
git tag -a "$TAG" -m "Research Librarian v${VERSION} — Document Intelligence & Scholarly Parsing"
git push origin main; git push origin "$TAG"
printf '\nResearch Librarian v%s validated and pushed successfully.\n' "$VERSION"
printf 'Deploy the backend package to Contabo before installing the WordPress v8.5.0 package.\n'
