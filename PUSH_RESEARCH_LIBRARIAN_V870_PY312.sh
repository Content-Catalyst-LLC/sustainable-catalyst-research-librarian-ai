...
ZIP_PATH="${1:-$SCRIPT_DIR/sustainable-catalyst-research-librarian-ai-v8.7.0-repository.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v870-clean}"
fail(){ printf 'ERROR: %s\n' "$*" >&2; exit 1; }
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v870.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
...
git add -A; git diff --cached --quiet&&fail "No v8.7.0 changes detected."
git commit -m "Build Research Librarian v${VERSION} — Core Evidence Bridge"
git tag -a "$TAG" -m "Research Librarian v${VERSION} — Core Evidence Bridge"
git push origin main; git push origin "$TAG"
printf '\nResearch Librarian v%s validated and pushed successfully.\n' "$VERSION"
printf 'Deploy the backend package to Contabo before installing the WordPress v8.7.0 package.\n'
