...
SCRIPT_REVISION="v650-py312-public-workspace-r1"
REPO_URL="git@github.com:Content-Catalyst-LLC/sustainable-catalyst-research-librarian-ai.git"
ZIP_PATH="${1:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-v6.5.0.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v650-clean}"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v650.XXXXXX")"
...
[[ -f "$SOURCE_DIR/tests/production-public-workspace-contract-test.php" ]] || fail "The v6.5.0 public workspace contract test is missing."
[[ -f "$SOURCE_DIR/data/research_librarian_public_workspace_manifest_v6.5.0.json" ]] || fail "The v6.5.0 public workspace manifest is missing."
...
grep -q 'Version: 6.5.0' "$SOURCE_DIR/sustainable-catalyst-research-librarian-ai.php" || fail "The package version marker is not 6.5.0."
grep -q 'data-workspace-version="1.0"' "$SOURCE_DIR/sustainable-catalyst-research-librarian-ai.php" || fail "The public workspace marker is missing."
...
grep -q 'sc-research-librarian-public-workspace/1.0' "$SOURCE_DIR/backend/app/main.py" || fail "The public workspace response schema is missing."
...
grep -q 'SCHEMA_VERSION = 6' "$SOURCE_DIR/backend/app/store.py" || fail "The package does not preserve SQLite schema version 6."
...
git tag -a "$RELEASE_TAG" -m "Research Librarian v${RELEASE_VERSION} — Production Public Research Workspace"
