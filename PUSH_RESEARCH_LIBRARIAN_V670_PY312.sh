...
SCRIPT_REVISION="v670-py312-research-quality-governance-r1"
REPO_URL="git@github.com:Content-Catalyst-LLC/sustainable-catalyst-research-librarian-ai.git"
ZIP_PATH="${1:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-v6.7.0.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v670-clean}"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v670.XXXXXX")"
...
[[ -f "$SOURCE_DIR/tests/research-quality-governance-contract-test.php" ]] || fail "The v6.7.0 reliability contract test is missing."
[[ -f "$SOURCE_DIR/tests/research-quality-governance-functional-test.php" ]] || fail "The v6.7.0 WordPress governance functional test is missing."
[[ -f "$SOURCE_DIR/backend/tests/test_governance.py" ]] || fail "The v6.7.0 backend reliability tests are missing."
[[ -f "$SOURCE_DIR/data/research_librarian_quality_governance_manifest_v6.7.0.json" ]] || fail "The v6.7.0 reliability manifest is missing."
[[ -f "$SOURCE_DIR/docs/V670_RESEARCH_QUALITY_GOVERNANCE_CENTER.md" ]] || fail "The v6.7.0 release documentation is missing."
[[ -f "$SOURCE_DIR/pytest.ini" ]] || fail "The package does not contain repository-root pytest configuration."
grep -q 'Version: 6.7.0' "$SOURCE_DIR/sustainable-catalyst-research-librarian-ai.php" || fail "The package version marker is not 6.7.0."
...
grep -q 'SCHEMA_VERSION = 9' "$SOURCE_DIR/backend/app/store.py" || fail "The package does not declare SQLite schema version 9."
...
git tag -a "$RELEASE_TAG" -m "Research Librarian v${RELEASE_VERSION} — Research Quality and Governance Center"
