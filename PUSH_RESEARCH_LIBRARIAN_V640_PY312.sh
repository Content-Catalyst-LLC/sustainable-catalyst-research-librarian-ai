...
SCRIPT_REVISION="v640-py312-hybrid-retrieval-r1"
REPO_URL="git@github.com:Content-Catalyst-LLC/sustainable-catalyst-research-librarian-ai.git"
ZIP_PATH="${1:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-v6.4.0.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v640-clean}"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v640.XXXXXX")"
...
[[ -f "$SOURCE_DIR/tests/hybrid-retrieval-citation-contract-test.php" ]] || fail "The v6.4.0 hybrid retrieval contract test is missing."
[[ -f "$SOURCE_DIR/data/research_librarian_hybrid_retrieval_manifest_v6.4.0.json" ]] || fail "The v6.4.0 release manifest is missing."
[[ -f "$SOURCE_DIR/pytest.ini" ]] || fail "The package does not contain repository-root pytest configuration."
grep -q 'Version: 6.4.0' "$SOURCE_DIR/sustainable-catalyst-research-librarian-ai.php" || fail "The package version marker is not 6.4.0."
grep -q 'SCHEMA_VERSION = 5' "$SOURCE_DIR/backend/app/store.py" || fail "The package does not declare SQLite schema version 5."
...
git tag -a "$RELEASE_TAG" -m "Research Librarian v${RELEASE_VERSION} — Hybrid Retrieval and Citation Engine"
