...
VERSION="10.1.0"; TAG="v${VERSION}"
REPO_URL="git@github.com:Content-Catalyst-LLC/sustainable-catalyst-research-librarian-ai.git"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ZIP_PATH="${1:-$SCRIPT_DIR/sustainable-catalyst-research-librarian-ai-v10.1.0-repository.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v1010-clean}"
fail(){ printf 'ERROR: %s\n' "$*" >&2; exit 1; }
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v1010.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
...
for f in \
 ...
 deploy/contabo/upgrade_research_librarian_backend_v10_1_0_contabo.sh; do [[ -f "$SRC/$f" ]]||fail "required file missing: $f"; done
grep -q 'Version: 10.1.0' "$SRC/sustainable-catalyst-research-librarian-ai.php"||fail "Plugin version mismatch."
grep -q '__version__ = "10.1.0"' "$SRC/backend/app/__init__.py"||fail "Backend version mismatch."
...
printf '=== VALIDATING Research Librarian v10.1.0 ===\n'
...
git add -A
git commit -m "Build Research Librarian v10.1.0 — Research Question & Hypothesis Intelligence"
git push origin main
if git rev-parse "$TAG" >/dev/null 2>&1; then fail "Tag $TAG already exists locally."; fi
git tag -a "$TAG" -m "Research Librarian v10.1.0 — Research Question & Hypothesis Intelligence"
git push origin "$TAG"
printf 'Research Librarian v10.1.0 validated and pushed successfully.\nDeploy the backend package to Contabo before installing the WordPress v10.1.0 package.\n'
