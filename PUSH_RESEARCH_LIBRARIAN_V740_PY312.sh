...
CANONICAL_ZIP_NAME="sustainable-catalyst-research-librarian-ai-v7.4.0-repository.zip"
ZIP_PATH="${1:-$SCRIPT_DIR/$CANONICAL_ZIP_NAME}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v740-clean}"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v740.XXXXXX")"
...
[[ -f "$SOURCE_DIR/backend/app/research_state.py" ]] || fail "The v7.4.0 research-state backend module is missing."
[[ -f "$SOURCE_DIR/backend/app/evidence_quality.py" ]] || fail "The preserved v7.3.0 evidence-quality backend module is missing."
[[ -f "$SOURCE_DIR/docs/V740_PERSISTENT_RESEARCH_STATE_READING_HISTORY_OPEN_QUESTIONS.md" ]] || fail "The v7.4.0 release documentation is missing."
[[ -f "$SOURCE_DIR/data/research_librarian_persistent_research_state_manifest_v7.4.0.json" ]] || fail "The v7.4.0 research-state manifest is missing."
[[ -f "$SOURCE_DIR/data/research_librarian_source_evaluation_manifest_v7.3.0.json" ]] || fail "The historical v7.3.0 evidence-quality manifest is missing."
[[ -f "$SOURCE_DIR/tests/v740-persistent-research-state-contract-test.php" ]] || fail "The v7.4.0 WordPress contract test is missing."
[[ -f "$SOURCE_DIR/backend/tests/test_v740_persistent_research_state.py" ]] || fail "The v7.4.0 backend test suite is missing."
grep -q 'Version: 7.4.0' "$SOURCE_DIR/sustainable-catalyst-research-librarian-ai.php" || fail "The plugin version marker is not 7.4.0."
grep -q '__version__ = "7.4.0"' "$SOURCE_DIR/backend/app/__init__.py" || fail "The backend version marker is not 7.4.0."
grep -q 'SCHEMA_VERSION = 14' "$SOURCE_DIR/backend/app/store.py" || fail "Ancillary SQLite schema 14 is missing."
...
grep -q 'sc-evidence-comparison/1.0' "$SOURCE_DIR/backend/app/evidence_quality.py" || fail "The preserved v7.3.0 evidence comparison contract is missing."
grep -q 'sc-evidence-gap-report/1.0' "$SOURCE_DIR/backend/app/evidence_quality.py" || fail "The preserved v7.3.0 evidence-gap contract is missing."
grep -q 'sc-connected-research-api/1.3' "$SOURCE_DIR/backend/app/platform_v7.py" || fail "Connected Research API 1.3 is missing."
grep -q 'sc-research-librarian-public-workspace/2.3' "$SOURCE_DIR/backend/app/main.py" || fail "Public workspace contract 2.3 is missing."
grep -q 'data-sc-rl-v730-quality-run' "$SOURCE_DIR/includes/class-sc-rl-v700-connected-platform.php" || fail "The preserved evidence-quality workspace action is missing."
grep -q 'data-sc-rl-v740-state-run' "$SOURCE_DIR/includes/class-sc-rl-v700-connected-platform.php" || fail "The v7.4.0 Research state workspace action is missing."
grep -q 'sc-research-state-summary/1.0' "$SOURCE_DIR/backend/app/research_state.py" || fail "The v7.4.0 research-state summary contract is missing."
grep -q 'sc-research-open-question/1.0' "$SOURCE_DIR/backend/app/research_state.py" || fail "The v7.4.0 open-question contract is missing."
grep -q 'rejected_context_objects_deprioritized' "$SOURCE_DIR/backend/app/main.py" || fail "Rejected-context-object retrieval deprioritization is missing."
grep -q 'workflow_memory_only' "$SOURCE_DIR/backend/app/main.py" || fail "The workflow-memory governance boundary is missing."
...
bash -n PUSH_RESEARCH_LIBRARIAN_V740_PY312.sh
git add -A
if git diff --cached --quiet; then fail "No v7.4.0 changes were detected after extraction."; fi
git commit -m "Build Research Librarian v${RELEASE_VERSION}"
git tag -a "$RELEASE_TAG" -m "Research Librarian v${RELEASE_VERSION} — Persistent Research State, Reading History & Open Questions"
git push origin main
git push origin "$RELEASE_TAG"

printf '\nResearch Librarian v%s validated and pushed successfully.\n' "$RELEASE_VERSION"
printf 'Render should auto-deploy main. Then update WordPress with the v7.4.0 WordPress package and verify Research state against a saved Library/project context.\n'
