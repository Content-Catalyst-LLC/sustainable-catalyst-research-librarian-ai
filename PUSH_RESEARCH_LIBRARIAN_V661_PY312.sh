...
RELEASE_VERSION="6.6.1"
RELEASE_TAG="v${RELEASE_VERSION}"
SCRIPT_REVISION="v661-py312-cross-product-reliability-r1"
REPO_URL="git@github.com:Content-Catalyst-LLC/sustainable-catalyst-research-librarian-ai.git"
ZIP_PATH="${1:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-v6.6.1.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v661-clean}"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v661.XXXXXX")"
...
[[ -f "$SOURCE_DIR/tests/cross-product-reliability-contract-test.php" ]] || fail "The v6.6.1 reliability contract test is missing."
[[ -f "$SOURCE_DIR/tests/cross-product-reliability-functional-test.php" ]] || fail "The v6.6.1 WordPress reliability functional test is missing."
[[ -f "$SOURCE_DIR/backend/tests/test_cross_product_reliability.py" ]] || fail "The v6.6.1 backend reliability tests are missing."
[[ -f "$SOURCE_DIR/data/research_librarian_cross_product_reliability_manifest_v6.6.1.json" ]] || fail "The v6.6.1 reliability manifest is missing."
[[ -f "$SOURCE_DIR/docs/V661_CROSS_PRODUCT_RELIABILITY_PATCH.md" ]] || fail "The v6.6.1 release documentation is missing."
[[ -f "$SOURCE_DIR/pytest.ini" ]] || fail "The package does not contain repository-root pytest configuration."
grep -q 'Version: 6.6.1' "$SOURCE_DIR/sustainable-catalyst-research-librarian-ai.php" || fail "The package version marker is not 6.6.1."
grep -q 'data-workspace-version="1.2"' "$SOURCE_DIR/sustainable-catalyst-research-librarian-ai.php" || fail "The public workspace marker is missing."
...
grep -q 'sc-research-librarian-public-workspace/1.2' "$SOURCE_DIR/backend/app/main.py" || fail "The public workspace response schema is missing."
...
grep -q 'SCHEMA_VERSION = 8' "$SOURCE_DIR/backend/app/store.py" || fail "The package does not declare SQLite schema version 8."
...
git tag -a "$RELEASE_TAG" -m "Research Librarian v${RELEASE_VERSION} — Cross-Product Reliability Patch"
