...
ZIP_PATH="${1:-$SCRIPT_DIR/sustainable-catalyst-research-librarian-ai-v8.4.0-repository.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v840-clean}"
fail(){ printf 'ERROR: %s\n' "$*" >&2; exit 1; }
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v840.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
...
for f in backend/app/advanced_retrieval.py backend/tests/test_v840_advanced_retrieval.py docs/V840_ADVANCED_RETRIEVAL_RERANKING_ENGINE.md data/research_librarian_advanced_retrieval_manifest_v8.4.0.json tests/v840-advanced-retrieval-reranking-contract-test.php backend/app/async_jobs.py backend/app/clients/platform_core.py backend/migrations/004_async_document_processing_runtime.sql deploy/contabo/upgrade_research_librarian_backend_v8_4_0_contabo.sh; do [[ -f "$SRC/$f" ]]||fail "v8.4.0 file missing: $f"; done
grep -q 'Version: 8.4.0' "$SRC/sustainable-catalyst-research-librarian-ai.php"||fail "Plugin version mismatch."
grep -q '__version__ = "8.4.0"' "$SRC/backend/app/__init__.py"||fail "Backend version mismatch."
...
grep -q '/opt/sustainable-catalyst/research-librarian-ai' "$SRC/deploy/contabo/upgrade_research_librarian_backend_v8_4_0_contabo.sh"||fail "Correct VPS Research Librarian root missing."
...
bash -n deploy/contabo/upgrade_research_librarian_backend_v8_4_0_contabo.sh
...
git add -A; git diff --cached --quiet&&fail "No v8.4.0 changes detected."
git commit -m "Build Research Librarian v${VERSION} — advanced retrieval and reranking"
git tag -a "$TAG" -m "Research Librarian v${VERSION} — Advanced Retrieval & Reranking Engine"
git push origin main; git push origin "$TAG"
printf '\nResearch Librarian v%s validated and pushed successfully.\n' "$VERSION"
printf 'Deploy the backend package to Contabo before installing the WordPress v8.4.0 package.\n'
