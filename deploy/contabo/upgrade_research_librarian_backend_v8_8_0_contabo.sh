...
ARCHIVE="${1:-/tmp/sustainable-catalyst-research-librarian-backend-v8.8.0.zip}"
BACKUP_ROOT="/opt/sustainable-catalyst/backups/research-librarian-v870"
TMP="$(mktemp -d /tmp/sc-rl-v870.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
grep -q '__version__ = "8.8.0"' "$INIT" || fail "backend package version mismatch"
...
s=re.sub(r'(?m)^(\s*image:\s*sustainable-catalyst-research-librarian:).*$', r'\g<1>8.8.0', s)
...
assert x.get('version')=='8.8.0',x
assert x.get('ready') is True,x
PY
    then break; fi
  fi
  if [[ "$i" == 90 ]]; then docker logs --tail=240 "$CONTAINER" >&2 || true; cat "$TMP/health.json" >&2 2>/dev/null || true; fail "Research Librarian v8.8.0 did not become ready"; fi
  sleep 2
done
python3 - "$TMP/health.json" <<'PY'
import json,sys
x=json.load(open(sys.argv[1])); assert x.get('version')=='8.8.0' and x.get('ready') is True,x
print('PASS: Research Librarian /health reports 8.8.0 and ready=true')
PY
...
echo "=== VERIFY v8.8 RESEARCH OBJECT SYNCHRONIZATION + v8.7 EVIDENCE BRIDGE + SOURCE/CORE/JOB/RETRIEVAL ROUTES ==="
docker exec -i "$CONTAINER" python - <<'PY'
...
echo "=== VERIFY v8.8 CORE RESEARCH OBJECT SYNCHRONIZATION ==="
docker exec -i "$CONTAINER" python - <<'PY'
...
print('PASS: v8.8 Core Research Object Synchronization routes and governance boundaries active')
PY
...
cap=data.get('capabilities',{}).get('project-state',{})
...
echo "PASS: Research Librarian AI v${VERSION} Core Research Object Synchronization backend deployed and verified."
