...
BACKUP_ROOT="/opt/sustainable-catalyst/backups/research-librarian-v820"
TMP="$(mktemp -d /tmp/sc-rl-v820.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
grep -q '__version__ = "8.2.0"' "$INIT" || fail "backend package version mismatch"
...
s=re.sub(r'(?m)^(\s*image:\s*sustainable-catalyst-research-librarian:).*$', r'\g<1>8.2.0', s)
p.write_text(s)
PY
...
import json,sys
x=json.load(open(sys.argv[1])); assert x.get('version')=='8.2.0',x
print('PASS: Research Librarian /health reports 8.2.0')
PY
...
echo "=== VERIFY v8.2 ROUTES + BINDING SCHEMA ==="
docker exec -i "$CONTAINER" python - <<'PY'
...
'/v1/core/exchange/packages'}
missing=required-paths
assert not missing,missing
assert SCHEMA_VERSION==19,SCHEMA_VERSION
print('PASS: v8.2 Core integration routes registered')
