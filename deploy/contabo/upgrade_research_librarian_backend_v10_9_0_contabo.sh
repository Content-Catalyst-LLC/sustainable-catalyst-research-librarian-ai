...
ARCHIVE="${1:-/tmp/sustainable-catalyst-research-librarian-backend-v10.9.0.zip}"
TMP="$(mktemp -d /tmp/sc-rl-v1090.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
grep -q '__version__ = "10.9.0"' "$INIT" || fail "backend package version mismatch"
for f in \
 ...
 app/contracts/computational_research_planning.py app/services/computational_research_planning.py tests/test_v1090_computational_research_planning.py migrations/024_computational_research_planning.sql; do
...
python3 - "$COMPOSE" <<'PYCOMPOSE1090'
from pathlib import Path
import re,sys
p=Path(sys.argv[1]); s=p.read_text()
s=re.sub(r'(?m)^(\s*image:\s*sustainable-catalyst-research-librarian:).*$', r'\g<1>10.9.0', s)
s=re.sub(r'(?m)^(\s*SC_RL_RELEASE_VERSION:\s*)["\']?[^"\'\n]+["\']?\s*$', r'\g<1>"10.9.0"', s)
p.write_text(s)
PYCOMPOSE1090
python3 - "$ENV_FILE" "$CORE_ENV" <<'PYENV1090'
from pathlib import Path
import sys
def parse(path):
    out={}
...
updates={'SC_RL_RELEASE_VERSION':'10.9.0','SC_RL_CORE_ENABLED':'true','SC_RL_CORE_BASE_URL':'http://sc-core:8090','SC_RL_CORE_MINIMUM_VERSION':'3.3.0','SC_RL_CORE_SUPPORTED_MAJOR':'3','SC_RL_CORE_FAIL_CLOSED_WRITES':'true','SC_RL_ASYNC_JOBS_ENABLED':'true'}
...
for i in $(seq 1 90); do
  if curl -fsS http://127.0.0.1:8093/health >"$TMP/health.json" 2>/dev/null && python3 - "$TMP/health.json" <<'PYHEALTH1090' >/dev/null 2>&1
import json,sys
x=json.load(open(sys.argv[1])); assert x.get('version')=='10.9.0' and x.get('ready') is True
PYHEALTH1090
  then break; fi
  if [[ "$i" == 90 ]]; then docker logs --tail=240 "$CONTAINER" >&2 || true; cat "$TMP/health.json" >&2 2>/dev/null || true; fail "Research Librarian v10.9.0 did not become ready"; fi
  sleep 2
done
python3 - "$TMP/health.json" <<'PYHEALTHPRINT1090'
import json,sys
x=json.load(open(sys.argv[1])); assert x.get('version')=='10.9.0' and x.get('ready') is True,x
print('PASS: Research Librarian /health reports 10.9.0 and ready=true')
PYHEALTHPRINT1090

echo "=== VERIFY PLATFORM CORE INTEGRATION ==="
docker exec -i "$CONTAINER" python - <<'PYCORE1090'
import asyncio
...
docker exec -i "$CONTAINER" python - <<'PYJOBS1090'
from app.async_jobs import JOB_TYPES
for job in ['dataset-discovery-data-fitness-snapshot','computational-research-planning-snapshot']: assert job in JOB_TYPES
print('PASS: v10.8/v10.9 durable snapshot jobs registered')
PYJOBS1090

echo "PASS: Research Librarian AI v10.9.0 Computational Research Planning backend deployed and verified."
