...
ARCHIVE="${1:-/tmp/sustainable-catalyst-research-librarian-backend-v9.5.0.zip}"
TMP="$(mktemp -d /tmp/sc-rl-v940.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
grep -q '__version__ = "9.5.0"' "$INIT" || fail "backend package version mismatch"
for f in \
 ...
 app/contracts/scholarly_publication.py app/services/scholarly_publication.py tests/test_v940_scholarly_publication.py migrations/009_scholarly_publication_dissemination.sql; do
...
s=re.sub(r'(?m)^(\s*image:\s*sustainable-catalyst-research-librarian:).*$', r'\g<1>9.5.0', s)
...
updates={'SC_RL_RELEASE_VERSION':'9.5.0','SC_RL_CORE_ENABLED':'true','SC_RL_CORE_BASE_URL':'http://sc-core:8090','SC_RL_CORE_MINIMUM_VERSION':'3.3.0','SC_RL_CORE_SUPPORTED_MAJOR':'3','SC_RL_CORE_FAIL_CLOSED_WRITES':'true','SC_RL_ASYNC_JOBS_ENABLED':'true'}
...
import json,sys
x=json.load(open(sys.argv[1])); assert x.get('version')=='9.5.0' and x.get('ready') is True
PY
  then break; fi
  if [[ "$i" == 90 ]]; then docker logs --tail=240 "$CONTAINER" >&2 || true; cat "$TMP/health.json" >&2 2>/dev/null || true; fail "Research Librarian v9.5.0 did not become ready"; fi
  sleep 2
done
python3 - "$TMP/health.json" <<'PY'
import json,sys
x=json.load(open(sys.argv[1])); assert x.get('version')=='9.5.0' and x.get('ready') is True,x
print('PASS: Research Librarian /health reports 9.5.0 and ready=true')
PY
...
echo "PASS: Research Librarian AI v9.5.0 Research Knowledge Graph & Publication Intelligence backend deployed and verified."
