...
ARCHIVE="${1:-/tmp/sustainable-catalyst-research-librarian-backend-v8.11.0.zip}"
BACKUP_ROOT="/opt/sustainable-catalyst/backups/research-librarian-v8110"
TMP="$(mktemp -d /tmp/sc-rl-v8110.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
grep -q '__version__ = "8.11.0"' "$INIT" || fail "backend package version mismatch"
...
[[ -f "$SRC_BACKEND/app/contracts/argument_synthesis.py" ]] || fail "v8.11 argument synthesis contract missing"
[[ -f "$SRC_BACKEND/app/services/argument_synthesis.py" ]] || fail "v8.11 argument synthesis service missing"
[[ -f "$SRC_BACKEND/tests/test_v8110_argument_synthesis.py" ]] || fail "v8.11 argument synthesis tests missing"
...
s=re.sub(r'(?m)^(\s*image:\s*sustainable-catalyst-research-librarian:).*$', r'\g<1>8.11.0', s)
...
assert x.get('version')=='8.11.0',x
assert x.get('ready') is True,x
PY
    then break; fi
  fi
  if [[ "$i" == 90 ]]; then docker logs --tail=240 "$CONTAINER" >&2 || true; cat "$TMP/health.json" >&2 2>/dev/null || true; fail "Research Librarian v8.11.0 did not become ready"; fi
  sleep 2
done
python3 - "$TMP/health.json" <<'PY'
import json,sys
x=json.load(open(sys.argv[1])); assert x.get('version')=='8.11.0' and x.get('ready') is True,x
print('PASS: Research Librarian /health reports 8.11.0 and ready=true')
PY
...
echo "=== VERIFY v8.11 ARGUMENT, CONTRADICTION & SYNTHESIS INTEGRATION ==="
docker exec -i "$CONTAINER" python - <<'PY'
import asyncio
...
print('PASS: v8.11 reviewer-directed argument planning and Core argument contract active')
PY


echo "=== VERIFY v8.11 STATISTICAL & ANALYTICAL RESEARCH INTEGRATION ==="
docker exec -i "$CONTAINER" python - <<'PY'
import asyncio
from app.main import app
from app.clients.platform_core import PlatformCoreClient
from app.services.statistical_research import capabilities
from app.contracts.statistical_research import CORE_STATISTICAL_CONTRACT
paths={getattr(r,'path',None) for r in app.routes}
required={'/v1/core/statistical-research/capabilities','/v1/core/statistical-research/readiness','/v1/core/statistical-research/plan','/v1/core/statistical-research/promote-validation','/v1/core/statistical-research/coefficients','/v1/core/statistical-research/intervals','/v1/core/statistical-research/interpretations'}
assert not (required-paths), required-paths
cap=capabilities(); assert cap['librarian_executes_arbitrary_analysis'] is False,cap
assert cap['automatic_significance_inference'] is False,cap
core=asyncio.run(PlatformCoreClient().statistical_reasoning_readiness())
assert core.get('contract')==CORE_STATISTICAL_CONTRACT,core
assert core.get('evidence_only') is True,core
assert core.get('human_review_required') is True,core
print('PASS: v8.11 statistical planning, specialist-runtime boundary, and Core statistical reasoning contract active')
PY
...
echo "PASS: Research Librarian AI v${VERSION} Statistical & Analytical Research Integration backend deployed and verified."
