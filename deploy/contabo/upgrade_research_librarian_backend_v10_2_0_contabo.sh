...
ARCHIVE="${1:-/tmp/sustainable-catalyst-research-librarian-backend-v10.2.0.zip}"
TMP="$(mktemp -d /tmp/sc-rl-v1020.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
grep -q '__version__ = "10.2.0"' "$INIT" || fail "backend package version mismatch"
for f in \
 ...
 app/contracts/research_design_methodology.py app/services/research_design_methodology.py tests/test_v1020_research_design_methodology.py migrations/017_research_design_methodology_planner.sql; do
...
s=re.sub(r'(?m)^(\s*image:\s*sustainable-catalyst-research-librarian:).*$', r'\g<1>10.2.0', s)
...
updates={'SC_RL_RELEASE_VERSION':'10.2.0','SC_RL_CORE_ENABLED':'true','SC_RL_CORE_BASE_URL':'http://sc-core:8090','SC_RL_CORE_MINIMUM_VERSION':'3.3.0','SC_RL_CORE_SUPPORTED_MAJOR':'3','SC_RL_CORE_FAIL_CLOSED_WRITES':'true','SC_RL_ASYNC_JOBS_ENABLED':'true'}
...
import json,sys
x=json.load(open(sys.argv[1])); assert x.get('version')=='10.2.0' and x.get('ready') is True
PY
  then break; fi
  if [[ "$i" == 90 ]]; then docker logs --tail=240 "$CONTAINER" >&2 || true; cat "$TMP/health.json" >&2 2>/dev/null || true; fail "Research Librarian v10.2.0 did not become ready"; fi
  sleep 2
done
python3 - "$TMP/health.json" <<'PY'
import json,sys
x=json.load(open(sys.argv[1])); assert x.get('version')=='10.2.0' and x.get('ready') is True,x
print('PASS: Research Librarian /health reports 10.2.0 and ready=true')
PY
...
echo "PASS: Research Librarian AI v10.2.0 Research Design & Methodology Planner backend deployed and verified."
