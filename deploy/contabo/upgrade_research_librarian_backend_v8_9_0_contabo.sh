...
ARCHIVE="${1:-/tmp/sustainable-catalyst-research-librarian-backend-v8.9.0.zip}"
BACKUP_ROOT="/opt/sustainable-catalyst/backups/research-librarian-v890"
TMP="$(mktemp -d /tmp/sc-rl-v890.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
grep -q '__version__ = "8.9.0"' "$INIT" || fail "backend package version mismatch"
...
s=re.sub(r'(?m)^(\s*image:\s*sustainable-catalyst-research-librarian:).*$', r'\g<1>8.9.0', s)
...
assert x.get('version')=='8.9.0',x
assert x.get('ready') is True,x
PY
    then break; fi
  fi
  if [[ "$i" == 90 ]]; then docker logs --tail=240 "$CONTAINER" >&2 || true; cat "$TMP/health.json" >&2 2>/dev/null || true; fail "Research Librarian v8.9.0 did not become ready"; fi
  sleep 2
done
python3 - "$TMP/health.json" <<'PY'
import json,sys
x=json.load(open(sys.argv[1])); assert x.get('version')=='8.9.0' and x.get('ready') is True,x
print('PASS: Research Librarian /health reports 8.9.0 and ready=true')
PY
...
echo "PASS: Research Librarian AI v${VERSION} Finding, Claim & Evidence Extraction Pipeline backend deployed and verified."
