...
ARCHIVE="${1:-/tmp/sustainable-catalyst-research-librarian-backend-v8.4.0.zip}"
BACKUP_ROOT="/opt/sustainable-catalyst/backups/research-librarian-v840"
TMP="$(mktemp -d /tmp/sc-rl-v840.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
grep -q '__version__ = "8.4.0"' "$INIT" || fail "backend package version mismatch"
...
import json,sys
x=json.load(open(sys.argv[1])); assert x.get('version')=='8.4.0',x
print('PASS: Research Librarian /health reports 8.4.0')
PY
