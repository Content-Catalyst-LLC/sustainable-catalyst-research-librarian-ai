...
ARCHIVE="${1:-/tmp/sustainable-catalyst-research-librarian-backend-v8.5.0.zip}"
BACKUP_ROOT="/opt/sustainable-catalyst/backups/research-librarian-v850"
TMP="$(mktemp -d /tmp/sc-rl-v850.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
grep -q '__version__ = "8.5.0"' "$INIT" || fail "backend package version mismatch"
...
s=re.sub(r'(?m)^(\s*image:\s*sustainable-catalyst-research-librarian:).*$', r'\g<1>8.4.0', s)
p.write_text(s)
PY
...
import json,sys
x=json.load(open(sys.argv[1])); assert x.get('version')=='8.5.0',x
print('PASS: Research Librarian /health reports 8.5.0')
PY
...
echo "=== VERIFY v8.4 ADVANCED RETRIEVAL + ASYNC JOB + CORE ROUTES ==="
docker exec -i "$CONTAINER" python - <<'PY'
...
print('PASS: v8.4 Core + async job + advanced retrieval routes registered')
