...
CANONICAL_ZIP_NAME="sustainable-catalyst-research-librarian-ai-v7.5.0-repository.zip"
ZIP_PATH="${1:-$SCRIPT_DIR/$CANONICAL_ZIP_NAME}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v750-clean}"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v750.XXXXXX")"
...
[[ -f "$SOURCE_DIR/backend/app/collaboration.py" ]] || fail "The v7.5.0 collaboration backend module is missing."
...
[[ -f "$SOURCE_DIR/docs/V750_COLLABORATIVE_RESEARCH_ROOM_INTELLIGENCE.md" ]] || fail "The v7.5.0 release documentation is missing."
[[ -f "$SOURCE_DIR/data/research_librarian_collaborative_room_manifest_v7.5.0.json" ]] || fail "The v7.5.0 collaborative-room manifest is missing."
[[ -f "$SOURCE_DIR/tests/v750-collaborative-research-room-intelligence-contract-test.php" ]] || fail "The v7.5.0 WordPress contract test is missing."
[[ -f "$SOURCE_DIR/backend/tests/test_v750_collaborative_research_rooms.py" ]] || fail "The v7.5.0 backend room test suite is missing."
[[ -f "$SOURCE_DIR/assets/sc-research-platform-v750-rooms.js" ]] || fail "The v7.5.0 collaborative-room browser runtime is missing."
grep -q 'Version: 7.5.0' "$SOURCE_DIR/sustainable-catalyst-research-librarian-ai.php" || fail "The plugin version marker is not 7.5.0."
grep -q '__version__ = "7.5.0"' "$SOURCE_DIR/backend/app/__init__.py" || fail "The backend version marker is not 7.5.0."
grep -q 'SCHEMA_VERSION = 15' "$SOURCE_DIR/backend/app/store.py" || fail "Ancillary SQLite schema 15 is missing."
...
grep -q 'sc-connected-research-api/1.4' "$SOURCE_DIR/backend/app/platform_v7.py" || fail "Connected Research API 1.4 is missing."
grep -q 'sc-research-librarian-public-workspace/2.4' "$SOURCE_DIR/backend/app/main.py" || fail "Public workspace contract 2.4 is missing."
...
grep -q 'data-sc-rl-v750-room-run' "$SOURCE_DIR/includes/class-sc-rl-v700-connected-platform.php" || fail "The v7.5.0 Research rooms workspace action is missing."
...
bash -n PUSH_RESEARCH_LIBRARIAN_V750_PY312.sh
git add -A
if git diff --cached --quiet; then fail "No v7.5.0 changes were detected after extraction."; fi
git commit -m "Build Research Librarian v${RELEASE_VERSION}"
git tag -a "$RELEASE_TAG" -m "Research Librarian v${RELEASE_VERSION} — Collaborative Research Room Intelligence"
git push origin main
git push origin "$RELEASE_TAG"

printf '\nResearch Librarian v%s validated and pushed successfully.\n' "$RELEASE_VERSION"
printf 'Render should auto-deploy main. Then update WordPress with the v7.5.0 WordPress package and smoke-test room membership, shared evidence, participant attribution, synthesis, and room-scoped Librarian context.\n'
