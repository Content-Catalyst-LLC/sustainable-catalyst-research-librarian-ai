...
def test_ask_creates_privacy_minimized_trace():
    ...
    assert trace['prompt_version']=='research-librarian-answer-v7.0.0'
...
def test_release_gate_blocks_critical_citation_failure_and_records_history():
    ...
    result=client.post('/v1/governance/release-gate',headers=HEADERS,json={'release_version':'7.0.0-test','metrics':metrics,'persist':True})
    ...
    assert any(item['release_version']=='7.0.0-test' for item in history)
...
def test_release_override_requires_named_human():
    ...
    missing=client.post('/v1/governance/release-gate',headers=HEADERS,json={'release_version':'7.0.0','metrics':metrics,'override':True})
    assert missing.status_code==409
    approved=client.post('/v1/governance/release-gate',headers=HEADERS,json={'release_version':'7.0.0','metrics':metrics,'override':True,'reviewer':'Release Reviewer'})
