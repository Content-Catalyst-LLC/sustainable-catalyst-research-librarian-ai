...
""")
    ...
    def lineage(self,eid:str)->dict[str,Any]:
        ...
        out["research_design"]["systematic_reviews"]=[]
        for ref in self._refs(rec,"systematic-review-plan"):
            try:
                rs=self._store("systematic_review_store",get_systematic_review_evidence_synthesis_store)
                out["research_design"]["systematic_reviews"].append({"review":rs.get(ref),"readiness":rs.readiness(ref),"flow":rs.flow(ref)})
            except Exception as exc: out["unresolved"].append({"component_type":"systematic-review-plan","ref":ref,"error":str(exc)})
        out["research_design"]["literature_intelligence"]=[]
        for ref in self._refs(rec,"literature-intelligence-plan"):
            try:
                ls=self._store("literature_intelligence_store",get_scholarly_literature_intelligence_store)
                out["research_design"]["literature_intelligence"].append({"project":ls.get(ref),"readiness":ls.readiness(ref),"citation_matrix":ls.citation_matrix(ref)})
            except Exception as exc: out["unresolved"].append({"component_type":"literature-intelligence-plan","ref":ref,"error":str(exc)})
        out["research_design"]["argument_intelligence"]=[]
        for ref in self._refs(rec,"argument-intelligence-plan"):
            try:
                ars=self._store("argument_intelligence_store",get_argument_claim_counterclaim_intelligence_store)
                out["research_design"]["argument_intelligence"].append({"project":ars.get(ref),"readiness":ars.readiness(ref),"argument_map":ars.argument_map(ref),"claim_evidence_matrix":ars.claim_evidence_matrix(ref)})
            except Exception as exc: out["unresolved"].append({"component_type":"argument-intelligence-plan","ref":ref,"error":str(exc)})
        out["research_design"]["research_gap_novelty_intelligence"]=[]
        for ref in self._refs(rec,"research-gap-novelty-plan"):
            try:
        ...
        out["research_design"]["computational_research_plans"]=[]
        for ref in self._refs(rec,"computational-research-plan"):
            try:
                cps=self._store("computational_research_planning_store",get_computational_research_planning_store)
                out["research_design"]["computational_research_plans"].append({"plan":cps.get(ref),"readiness":cps.readiness(ref),"execution_graph":cps.execution_graph(ref),"execution_handoffs":cps.execution_handoffs(ref)})
            except Exception as exc: out["unresolved"].append({"component_type":"computational-research-plan","ref":ref,"error":str(exc)})
        out["research_design"]["research_programs"]=[]
        for ref in self._refs(rec,"research-program"):
            try:
                rps=self._store("research_program_store",get_research_program_intelligence_store)
                out["research_design"]["research_programs"].append({"program":rps.get(ref),"readiness":rps.readiness(ref),"program_graph":rps.program_graph(ref),"portfolio_summary":rps.portfolio_summary(ref)})
            except Exception as exc: out["unresolved"].append({"component_type":"research-program","ref":ref,"error":str(exc)})
    ...
    def readiness(self,eid:str)->dict[str,Any]:
        ...
        dimensions={
            ...
            "systematic_review_bound":bool(self._refs(rec,"systematic-review-plan")),
