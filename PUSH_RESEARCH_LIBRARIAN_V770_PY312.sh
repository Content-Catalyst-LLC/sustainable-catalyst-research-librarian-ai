...
RELEASE_VERSION="7.7.0"
RELEASE_TAG="v${RELEASE_VERSION}"
REPO_URL="git@github.com:Content-Catalyst-LLC/sustainable-catalyst-research-librarian-ai.git"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ZIP_PATH="${1:-$SCRIPT_DIR/sustainable-catalyst-research-librarian-ai-v7.7.0-repository.zip}"
REPO_DIR="${2:-$HOME/Downloads/sustainable-catalyst-research-librarian-ai-repo-v770-clean}"
fail(){ printf 'ERROR: %s\n' "$*" >&2; exit 1; }
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-rl-v770.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
...
for f in backend/app/federated_discovery.py backend/tests/test_v770_federated_research.py docs/V770_GLOBAL_LIBRARY_DISCOVERY_FEDERATED_RESEARCH.md data/research_librarian_federated_discovery_manifest_v7.7.0.json tests/v770-global-library-discovery-federated-research-contract-test.php assets/sc-research-platform-v770-federation.js; do [[ -f "$SRC/$f" ]]||fail "v7.7.0 file missing: $f"; done
grep -q 'Version: 7.7.0' "$SRC/sustainable-catalyst-research-librarian-ai.php"||fail "Plugin version mismatch."
grep -q '__version__ = "7.7.0"' "$SRC/backend/app/__init__.py"||fail "Backend version mismatch."
grep -q 'SCHEMA_VERSION = 17' "$SRC/backend/app/store.py"||fail "Ancillary SQLite schema 17 missing."
...
grep -q 'sc-connected-research-api/1.6' "$SRC/backend/app/platform_v7.py"||fail "API 1.6 missing."
grep -q 'sc-research-librarian-public-workspace/2.6' "$SRC/backend/app/main.py"||fail "Workspace 2.6 missing."
grep -q 'external-reference' "$SRC/backend/app/federated_discovery.py"||fail "External-reference import boundary missing."
grep -q 'arbitrary_remote_urls_forbidden' "$SRC/backend/app/federated_discovery.py"||fail "Fixed-provider boundary missing."
grep -q 'data-sc-rl-v770-federation-run' "$SRC/includes/class-sc-rl-v700-connected-platform.php"||fail "Discover globally UI missing."
...
bash -n PUSH_RESEARCH_LIBRARIAN_V770_PY312.sh
git add -A; git diff --cached --quiet&&fail "No v7.7.0 changes detected."
git commit -m "Build Research Librarian v${RELEASE_VERSION}"
git tag -a "$RELEASE_TAG" -m "Research Librarian v${RELEASE_VERSION} — Global Library Discovery & Federated Research"
git push origin main; git push origin "$RELEASE_TAG"
printf '\nResearch Librarian v%s validated and pushed successfully.\n' "$RELEASE_VERSION"
printf 'Render should auto-deploy main. Then install the v7.7.0 WordPress package and smoke-test federated search, provider failures, recent history, explicit Save to My Library, and external-reference provenance.\n'
